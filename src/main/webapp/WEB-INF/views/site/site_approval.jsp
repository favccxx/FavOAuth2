...
<head>
  ...
  <script type="text/javascript">
  	function approveAppSite(appState){
  		
  		var appId = $('input[name="favWebSiteRadio"]:checked').val();
  		var data = {
  				appId : appId,
  				appState : appState
  		};
  		...
  		$.ajax({
			type : "POST",			
			url : "<%=basePath%>site/approveAppSite",
			data : data,
			dataType : 'JSON',
			timeout : 100000,
      ...
      <div class="am-u-sm-12 am-u-md-6">
        <div class="am-btn-toolbar">
          <div class="am-btn-group am-btn-group-xs">
          	<button type="button" class="am-btn am-btn-default" onclick="approveAppSite('APPROVE');"><span class="am-icon-plus"></span> 审核通过</button>
     		<button type="button" class="am-btn am-btn-default" onclick="approveAppSite('REJECT');"><span class="am-icon-save"></span> 审核不通过</button>          	
          </div>
        </div>
      </div>
        ...
        <form class="am-form">
          ...
          <tbody>
          	<c:forEach items="${appList }" var="appItem">
          		<tr>
          			<td><input type="radio" name="favWebSiteRadio" value="${appItem.id }"/></td>
